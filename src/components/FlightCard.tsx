...
export function FlightCard({ offer, onFavorite, favorite }: Props) {
  return <article className="flight-card">
    ...
    <div className="flight-main"><div><strong>{offer.airline}</strong><small>{offer.flightNumber}</small></div><div className="route"><b>{new Date(offer.departure).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</b><span><Plane size={18}/>{offer.stops ? `${offer.stops} escala` : 'Direto'}</span><b>{new Date(offer.arrival).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</b></div><div className="price"><del>{money(offer.originalPrice)}</del><strong>{money(offer.price)}</strong><small>por passageiro</small>{Number.isFinite(offer.totalPrice)&&offer.totalPrice!==offer.price&&<small><b>{money(offer.totalPrice)}</b> total</small>}</div></div>
